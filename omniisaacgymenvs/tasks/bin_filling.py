...
class BinFilling(RLTask):
    ...
    def __init__(self,
        ...
        self._screw_asset_paths = [
            ...
            "/Isaac/Props/Flip_Stack/large_corner_bracket_physics.usd",
            self._assets_root_path +
            "/Isaac/Props/Flip_Stack/screw_95_physics.usd",
            self._assets_root_path +
            "/Isaac/Props/Flip_Stack/screw_99_physics.usd",
            self._assets_root_path +
            "/Isaac/Props/Flip_Stack/small_corner_bracket_physics.usd",
            self._assets_root_path +
            "/Isaac/Props/Flip_Stack/t_connector_physics.usd",
        ]
        self._screws = []
        ...
        self._bin_initial_position = np.array([0.35, 0.15, -0.40
                                               ]) / get_stage_units()
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        add_reference_to_stage(usd_path=self._ur10_asset_path,
        ...
        import torch
        self._ur10_robot.set_joints_default_state(positions=torch.as_tensor(
            [-np.pi / 2, -np.pi / 2, -np.pi / 2, -np.pi / 2, np.pi /
             2, 0]).to(self.device))
    ...
    def pre_physics_step(
        ...
        # if self._screws_to_add > 0 and len(
        #         self._screws) < self._max_screws and time_step_index % 30 == 0:
        #     self._add_screw()
        self._step += 1
        return
    ...
    def _add_screw(self):
        asset_path = self._screw_asset_paths[random.randint(
            0,
            len(self._screw_asset_paths) - 1)]
        prim_path = "/World/objects/object_{}".format(len(self._screws))
        add_reference_to_stage(usd_path=asset_path, prim_path=prim_path)
        self._screws.append(
            self.scene.add(
                XFormPrim(prim_path=prim_path,
                          name="screw_{}".format(len(self._screws)),
                          position=self._pipe_position)))
    ...
    def get_params(self) -> dict:
        ...
        params_representation["bin_name"] = {
            "value": self._packing_bin.name,
            "modifiable": False
        }
        params_representation["robot_name"] = {
            "value": self._ur10_robot.name,
            "modifiable": False
        }
