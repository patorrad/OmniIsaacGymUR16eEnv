    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor,
        ...
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
                next_non_terminal = torch.as_tensor(1.0 - np.array(dones).astype(int),device=self.device)
                next_values =  torch.as_tensor(last_values,device=self.device)
            else:
                next_non_terminal = torch.as_tensor(1.0 - self.episode_starts[step + 1],device=self.device)
                next_values = torch.as_tensor(self.values[step + 1],device=self.device)
...
class DictReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        assert isinstance(
            self.obs_shape,
            dict), "DictReplayBuffer must be used with Dict obs space only"
        self.buffer_size = max(buffer_size // n_envs, 1)
        ...
        self.observations = {
            key:
            np.zeros((self.buffer_size, self.n_envs, *_obs_shape),
    ...
    def add(  # type: ignore[override]
        ...
        for key in self.observations.keys():
            ...
            if isinstance(self.observation_space.spaces[key], spaces.Discrete):
                obs[key] = obs[key].reshape((self.n_envs, ) +
                                            self.obs_shape[key])
            self.observations[key][self.pos] = np.array(obs[key])

        for key in self.next_observations.keys():
            if isinstance(self.observation_space.spaces[key], spaces.Discrete):
                next_obs[key] = next_obs[key].reshape((self.n_envs, ) +
                                                      self.obs_shape[key])
    ...
    def _get_samples(  # type: ignore[override]
        ...
        next_observations = {
            key: self.to_torch(obs)
            for key, obs in next_obs_.items()
        }
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def __init__(
        ...
        assert isinstance(
            self.obs_shape,
            dict), "DictRolloutBuffer must be used with Dict obs space only"
